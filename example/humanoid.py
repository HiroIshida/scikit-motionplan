...
from skmp.solver.auxpoint_solver import AuxSolver, AuxSolverConfig
...
start_coords_list = [
    Coordinates([0, -0.2, 0]),
    Coordinates([0, +0.2, 0]),
    Coordinates([0, -0.3, 1.5], rot=rpy_matrix(0, angle, 0)),
    Coordinates([0, +0.3, 1.5], rot=rpy_matrix(0, angle, 0)),
]
...
solver = AuxSolver.init(AuxSolverConfig())
...
sqp_solver = SQPBasedSolver.init(
    SQPBasedSolverConfig(
        20, n_max_call=100, motion_step_satisfaction="explicit", verbose=True, ctol_eq=1e-3
    )
)
sqp_solver.setup(problem)
res = sqp_solver.solve(res.traj)
assert res.traj is not None
print(res.time_elapsed)

traj = res.traj.resample(10)


vis = TrimeshSceneViewer()
vis.add(jaxon)
for i in range(15):
    box = Box([0.1, 1.0, 0.05])
    box.translate([0.1, 0.0, -0.9 + 0.3 * i])
    vis.add(box)
vis.show()
time.sleep(15)
